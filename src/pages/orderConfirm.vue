...
export default{
  ...
  data () {
    return {
      ...
      cartTotalPrice: 0,//购物车商品总金额
      count: 0, //商品结算数量
      checkedItem: {},//选中的商品对象
      userAction: '',//用户行为 0：新增地址 1：编辑地址 2：删除地址
      showDelModal: false,//是否显示删除弹框
      showEditModal:false,//是否显示新增辑或者编辑弹框
  ...
  methods: {
    ...
    submitAddress () {
      ...
      //method变量为请求方式，url为请求地址，params为请求时携带的参数
      let method,url,params = {};
      //如果userAction == 0，表示此时事件为添加地址
      if (userAction == 0) {
        method = 'post', url = '/shippings';
      } else if (userAction == 1) { //此时事件为编辑地址
      //因为编辑和删除地址要获取id，checkedItem.id为新增地址时接口返回的特定id
      //这里的checkedItem.id是在该函数内新定义的checkedItem变量，用解构赋值的方式将this.checkedItem的值赋给checkedItem
      //故checkedItem.id的值为this.checkedItem.id  而this.checkedItem.id为delAddress()函数所获取到的id值
      ...
      if (userAction == 0 || userAction == 1) {
        ...
        //params为请求时携带的变量：receiverName ：receiverName 即 receiverName ： this.checkedItem.receiverName
        params = {
          receiverName,
          receiverMobile, 
          receiverProvince, 
    ...
    getCartList () {
      this.axios.get('/carts').then((res) => {
        ...
        this.cartTotalPrice = res.cartTotalPrice; //购物车商品总金额
        //filter为带有过滤功能的方法  返回item.productSelected为true的新数组
        this.cartList = list.filter(item => item.productSelected);//过滤出选中状态的商品，返回一个新数组给list
